/**
 * PilgrimSafe — Auto-Seed (in-process, Prisma)
 *
 * A lightweight version of seed.js that populates the already-connected
 * PostgreSQL database via Prisma. Called by server.js when the DB is empty.
 *
 * Does NOT disconnect or call process.exit().
 */
...
/**
 * Auto-seed the database with 4 temples and their zones.
 * Exported as an async function so server.js can await it.
 */
async function autoSeed() {
  for (const templeData of temples) {
    ...
    console.log(`  ✅ ${temple.name} (${temple.slug})`);
  ...
  console.log(`🎉 Auto-seeded ${temples.length} temples.`);
}
